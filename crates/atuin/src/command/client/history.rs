use std::{
    fmt::{self, Display},
    io::{self, IsTerminal, Result as IoResult, Write},
    time::Duration,
};
...
pub(super) async fn start_history_entry(
    ...
    if settings.daemon.enabled {
        if let Ok(id) = handle_daemon_start(settings, command, author, intent).await {
            return Ok(id);
        }
    }
...
pub(super) async fn end_history_entry(
    ...
    #[cfg(feature = "daemon")]
    if settings.daemon.enabled {
        if handle_daemon_end(settings, id, exit, duration).await.is_ok() {
            return Ok(());
        }
    }
